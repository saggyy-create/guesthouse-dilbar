...
import { calendarMeta } from '../data/calendars'
...
const ApartmentsAvailabilityCalendar = ({ calId }) => {
  ...
  const handleDayClick = (date) => {
    ...
    if (!draft.checkIn || (draft.checkIn && draft.checkOut)) {
      update({ checkIn: iso, checkOut: '', stayType: 'apartments' })
    } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
      ...
      <div className="text-center mb-4">
        <h3 className="text-xl font-display font-bold text-gray-900">{meta.label}</h3>
        {meta.description && (
          <p className="text-sm text-gray-600 font-body">{meta.description}</p>
        )}
      </div>
...
export default ApartmentsAvailabilityCalendar
